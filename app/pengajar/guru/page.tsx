...
export default function PengajarGuruPage() {
  ...
  return (
    <div className="mx-auto w-full max-w-6xl space-y-6 p-4 sm:p-6 lg:p-8">
      <div>
        <h1 className="font-[family-name:var(--font-display)] text-2xl font-semibold text-[var(--color-navy)]">Kelola Guru</h1>
        <p className="mt-1 text-sm text-gray-500">Daftar guru binaan di sekolah Anda (otomatis sesuai lingkup akun Anda).</p>
      </div>
      ...
      <div className="relative w-full sm:max-w-sm">
        ...
        <input
          ...
          className="w-full rounded-xl border border-[var(--color-border-soft)] bg-white py-2.5 pl-10 pr-10 text-sm text-gray-800 placeholder:text-gray-400 outline-none transition focus:border-[var(--color-navy)] focus:ring-2 focus:ring-[var(--color-navy)]/15"
        />
        {searchQuery && (
          <button
            type="button"
              ...
              {filtered.length === 0 ? (
                ...
                filtered.map((g) => (
                  <tr key={g.id} className="border-b border-[var(--color-border-soft)] last:border-0">
                    <td className="px-4 py-3 text-gray-800">{g.nama}</td>
                    <td className="px-4 py-3 text-gray-600">{g.sekolah || <span className="text-gray-400">—</span>}</td>
                    <td className="px-4 py-3 text-gray-600">{g.email}</td>
                    <td className="px-4 py-3 text-center">
                      {g.status ? (
                        <span className={`rounded-full px-2 py-0.5 text-xs font-medium capitalize ${statusColor[g.status] || "bg-gray-100 text-gray-600"}`}>
                          {g.status}
                        </span>
                      ) : (
                    ...
                    <td className="px-4 py-3 text-center">
                      <Link
                        ...
                        className="rounded-full border border-[var(--color-border-soft)] px-3 py-1.5 text-sm text-[var(--color-navy)] transition hover:bg-gray-50"
                      >
                        Detail
                      </Link>
                    </td>
