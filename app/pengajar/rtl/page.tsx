  ...
  return (
    <div className="mx-auto w-full max-w-6xl space-y-6 p-4 sm:p-6 lg:p-8">
      <div>
        ...
        <p className="mt-1 text-sm text-gray-500 dark:text-slate-400">{t("Tinjau dan nilai pengajuan RTL dari guru binaan Anda.", "Review and assess RTL submissions from your supervised teachers.")}</p>
      </div>
      ...
      <div className="overflow-hidden rounded-2xl border border-[var(--color-border-soft)] bg-white shadow-sm dark:bg-slate-900">
        {loading ? (
          ...
          <div className="py-16 text-center">
            <p className="text-sm text-gray-500 dark:text-slate-400">{t("Belum ada pengajuan RTL", "No RTL submissions yet")}{filterStatus ? t(" untuk status ini", " for this status") : ""}.</p>
          </div>
        ) : (
                  ...
                  return (
                    <tr key={r.id} className="transition hover:bg-slate-50/80 dark:hover:bg-slate-800/40">
                      <td className="px-4 py-3">
                        ...
                        <div className="text-xs text-gray-400 dark:text-slate-500">{r.user?.sekolah || ""}</div>
                      </td>
