...
export default function MaterialStatusBadge({ entry, className = "" }: MaterialStatusBadgeProps) {
  ...
  if (completed) {
    return (
      <span
        className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold bg-emerald-50 text-emerald-700 border border-emerald-200 ${className}`}
      >
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold bg-sky-50 text-sky-700 border border-sky-200 ${className}`}
    >
      {percent}%
    </span>
  );
