  ...
  return (
    <div className="mx-auto max-w-5xl p-6">
      <div className="mb-6 flex items-center justify-between gap-4">
        <div>
          <h1 className="font-[family-name:var(--font-display)] text-2xl font-medium text-[var(--color-navy)] dark:text-slate-100">{t("Kelola Course", "Manage Courses")}</h1>
          <p className="mt-1 text-sm text-gray-500 dark:text-slate-400">{t("Daftar course yang tersedia di sistem.", "List of courses available in the system.")}</p>
        </div>
        <Link
          href="/admin/courses/new"
          className="shrink-0 rounded-full bg-[var(--color-navy)] px-4 py-2 text-sm text-white transition hover:opacity-90"
        >
          {t("+ Tambah Course", "+ Add Course")}
        </Link>
      </div>
          ...
          {courses.map((course) => {
              ...
              <>
                <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-start">
                  <div className="min-w-0">
                    <h2 className="truncate font-medium text-[var(--color-navy)] dark:text-slate-100">{course.judul || t("Tanpa judul", "Untitled")}</h2>
                    <p className="mt-1 line-clamp-2 text-sm text-gray-500 dark:text-slate-400">{course.deskripsi || t("Tidak ada deskripsi.", "No description.")}</p>
                  </div>
                  <span className="w-fit rounded-full bg-slate-100 px-2.5 py-1 text-xs capitalize text-slate-600 dark:bg-slate-800 dark:text-slate-300">
                    {course.mode || "—"}
                  </span>
                </div>
                <div className="mt-4 flex flex-wrap gap-x-5 gap-y-2 text-xs text-gray-500 dark:text-slate-400">
                  <span>{course.hasCertificate ? t("Dengan sertifikat", "With certificate") : t("Tanpa sertifikat", "Without certificate")}</span>
                  <span>{t("Mulai:", "Start:")} {formatDate(course.tanggalMulai)}</span>
                  <span>{t("Selesai:", "End:")} {formatDate(course.tanggalSelesai)}</span>
                  {!manageable && (
                    <span className="text-amber-600 dark:text-amber-400">
                      {course.isGlobal || course.schoolId === null ? t("Course Global", "Global Course") : t("Course pengajar lain", "Another instructor's course")}
                    </span>
                  )}
                </div>
              </>
            ...
            return manageable ? (
              <Link
                ...
                className="rounded-2xl border border-[var(--color-border-soft)] bg-white p-5 transition hover:shadow-md dark:bg-slate-900 dark:hover:border-slate-700"
              >
                {content}
              </Link>
            ) : (
              <div
                key={course.id}
                className="rounded-2xl border border-[var(--color-border-soft)] bg-white/70 p-5 dark:bg-slate-900/70"
              >
                {content}
              </div>
            );
