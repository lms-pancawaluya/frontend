  ...
  return (
    <div className="mx-auto w-full max-w-6xl space-y-6 p-4 sm:p-6 lg:p-8">
      <div>
        ...
        <p className="mt-1 text-sm text-gray-500">Tinjau dan nilai pengajuan RTL dari seluruh guru.</p>
      </div>
      ...
      <div className="flex flex-wrap items-end gap-4">
        ...
        {sekolahOptions.length > 0 && (
          <select
            ...
            className="rounded-xl border border-slate-200 bg-white px-3 py-1.5 text-xs text-gray-700 outline-none transition focus:ring-2 focus:ring-[var(--color-navy)]/15"
          >
            ...
            <table className="w-full text-sm">
              <thead className="border-b border-[var(--color-border-soft)] bg-[var(--color-pale)]">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">Sekolah</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">Modul</th>
                  <th className="px-4 py-3 text-center text-xs font-semibold uppercase tracking-wider text-gray-500">Tanggal</th>
                ...
                {filtered.map((r) => {
                  ...
                  return (
                    <tr key={r.id} className="transition hover:bg-slate-50/80">
                      ...
                      <td className="px-4 py-3 text-gray-600">{r.user?.sekolah || "—"}</td>
                      <td className="px-4 py-3 text-gray-700">{r.module?.judul || "—"}</td>
                      <td className="px-4 py-3 text-center text-xs text-gray-500">{fmtDate(r.createdAt)}</td>
            ...
            <div className="flex-1 space-y-5 overflow-y-auto p-6">
              {detailLoading ? (
                ...
                <>
                  ...
                  {detail.createdAt && (
                    <p className="text-xs text-gray-400">Diajukan: {fmtDate(detail.createdAt)}</p>
                  )}
