      ...
      <div className="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white/90 backdrop-blur-md p-4 rounded-2xl border border-slate-200/80 shadow-sm dark:bg-slate-900/90 dark:border-slate-800">
          ...
          >
            ...
            {t("Kembali ke Pengantar Modul", "Back to Module Introduction")}
          </button>
                ...
                {!attemptResult ? (
                  <form onSubmit={handleSubmitQuiz} className="space-y-5">
                    <div className="border-b border-slate-100 pb-4 dark:border-slate-800">
                      ...
                      <h3 className="font-bold text-slate-800 text-base leading-snug dark:text-slate-100">{activeQuiz.judul}</h3>
                    </div>
                    ...
                    <div className="space-y-5 max-h-[55vh] overflow-y-auto pr-1">
                      {activeQuiz.questions?.length ? (
                        activeQuiz.questions.map((q, idx) => (
                          <div key={q.id} className="space-y-3">
                            <p className="font-semibold text-slate-800 text-sm leading-relaxed dark:text-slate-200">
                              <span className="text-emerald-700 font-bold dark:text-emerald-400">{idx + 1}.</span> {q.pertanyaan}
                            </p>
                                ...
                                return (
                                  ...
                                  >
                                    <div className={`flex-shrink-0 w-6 h-6 rounded-lg text-xs font-bold flex items-center justify-center transition-colors ${
                                      isSelected ? "bg-emerald-700 text-white" : "bg-slate-200 text-slate-600 dark:bg-slate-700 dark:text-slate-300"
                                    }`}>
                        ...
                        <p className="text-xs text-slate-400 text-center py-4 dark:text-slate-500">
                          {t("Memuat pertanyaan evaluasi...", "Loading evaluation questions...")}
                        </p>
                      )}
                    </div>
        ...
        <div className="bg-white/90 backdrop-blur-md p-5 rounded-2xl border border-slate-200/80 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4 dark:bg-slate-900/90 dark:border-slate-800">
          <div className="flex items-center gap-3">
            ...
            <p className="text-xs sm:text-sm text-slate-600 font-medium leading-relaxed dark:text-slate-300">
              {!isVideoFinished
                ? t("Selesaikan penayangan video dan kuis interaktif untuk melanjutkan ke modul teks.", "Complete the video playthrough and interactive quiz to continue to the text module.")
          ...
          >
            <span>{t("Lanjut ke Materi Teks", "Continue to Text Material")}</span>
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M14 5l7 7m0 0l-7 7m7-7H3" />
            </svg>
          </button>
