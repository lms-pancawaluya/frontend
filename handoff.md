...
- Responsive header: the shared header collapses to a hamburger menu at `<=800px` while keeping the same auth/role-aware routes and desktop navigation unchanged above `800px`.
...
app/
  ...
  admin/
    ...
    rtl/page.tsx             RTL review — all schools; status filter (API), sekolah/guru filters (client); detail modal + review (setujui/tolak); reuses rtl.service
    diskusi/page.tsx         Diskusi/Komentar Modul — global scope; read threads, post replies, delete comments for moderation; reuses comment.service
  pengajar/                  Pengajar panel (role: pengajar) — shared Sidebar shell, Admin design language, school-scoped
    ...
    diskusi/page.tsx         module picker + comments list (getModuleComments) + post (postComment); NO delete/moderation
services/                    auth, module, content, evaluation, miniQuiz, progress, user, checklist, helpdesk, rtl, comment (.js)
...
| `/admin/rtl` | RTL review (all schools) | ✅ | `getRtlSubmissions`, `getRtlDetail`, `reviewRtl` |
| `/admin/diskusi` | Module discussion & moderation | ✅ | `getModuleComments`, `postComment`, `deleteComment` |
...
| Admin Helpdesk / Ticketing | ✅ | `/admin/helpdesk` | `getAllTickets`, `getTicketDetail`, `replyToTicket`, `updateTicketStatus` | server-side filters (status/category), modal detail view, reply system, manual status change (PATCH); tiket Pengajar (requester `role` dari API) di-highlight + badge "Pengajar" + diurutkan ke atas |
| Admin RTL review | ✅ | `/admin/rtl` | `getRtlSubmissions`, `getRtlDetail`, `reviewRtl` | all-school scope (backend-determined); status filter (API), sekolah + guru name filters (client-side); detail modal + approve/reject with catatan |
| Admin Module Discussion | ✅ | `/admin/diskusi` | `getModuleComments`, `postComment`, `deleteComment` | global scope; view thread, post reply, delete comments |
...
| Helpdesk | `/api/helpdesk/tickets` (POST), `/api/helpdesk/tickets/my` (GET), `/api/helpdesk/tickets/:id` (GET), `/api/helpdesk/tickets/:id/replies` (POST) | | helpdesk.service (guru) |
