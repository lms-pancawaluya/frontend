...
> **Header responsif:** header bersama (floating header yang sama untuk Guru, Pengajar, dan Admin — search & notifikasi masih dummy/non-functional) kini tampil di **Landing Page** (`/`) dan **Dashboard tiap role** (`/dashboard`, `/admin`, `/pengajar`). Di landing, header memakai hamburger menu pada lebar `<=800px` dan navigasi auth/role yang sama (guest, guru, pengajar, admin); tampilan desktop di atas `800px` tetap tidak berubah. Pada dashboard role, floating header muncul di `≥801px` dan disembunyikan di `≤800px` (menggunakan Sidebar drawer/hamburger sebagai satu-satunya navigasi).
